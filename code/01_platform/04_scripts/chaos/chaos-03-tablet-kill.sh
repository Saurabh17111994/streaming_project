...
echo "TABLET-KILL-CHAOS-03: running TabletKillChaosIntegrationTest FLUSS_BOOTSTRAP=${FLUSS_BOOTSTRAP} TABLET_CONTAINER=${TABLET_CONTAINER} CHAOS_SCAN_LIMIT_ROWS=${CHAOS_SCAN_LIMIT_ROWS}"
if ! COMPUTE_INT_TEST_TABLET_KILL=true FLUSS_BOOTSTRAP="${FLUSS_BOOTSTRAP}" TABLET_CONTAINER="${TABLET_CONTAINER}" TABLET_KILL_ROWS="${TABLET_KILL_ROWS}" CHAOS_REPLICATION_REQUIRED="${CHAOS_REPLICATION_REQUIRED}" CHAOS_REPLICATION_MIN="${CHAOS_REPLICATION_MIN}" CHAOS_SCAN_LIMIT_ROWS="${CHAOS_SCAN_LIMIT_ROWS}" mvn -f "${COMPUTE_DIR}/pom.xml" -Dtest=TabletKillChaosIntegrationTest -DfailIfNoTests=false test 2>&1; then
