...
- [ ] `A2.2` Stand up compose with `--profile execution-t3`; confirm `gateway:9180/healthz` 200, `nautilus:9190/healthz` reports `HALTED`, `bridge:8787/healthz` reports `UP disabled` until approval. **Step 0 (CHG-101, 2026-08-25): `make check-image-stale` MUST PASS first** — the 2026-08-24 discovery (gateway/bridge images 08-20 vs source 08-24, stale gateway answered readyz 200 with pre-CHG-092 fail-open semantics) is exactly the failure mode the guard mechanises; rebuild with `docker compose --profile execution-t3 build` when it fails.
...
- **T4a landed 2026-08-24 (CHG-090):** the ENABLED `/v1/intents` route now maps the
  verified payload → bridge `place` (deterministic 14-hex `client_order_ref`, fresh UUID
  v4 attempt id) and forwards — SUCCESS→202 (broker id + ref echo), REJECTED→409,
  UNKNOWN/transport-err→503 (never ack, never retry). Auth 401 and HALTED 503 paths
  unchanged.
- **A2.4 lifecycle leg landed 2026-08-24 (CHG-091):** the SUCCESS 202 now emits the
  normalized lifecycle+correlation image to the gateway `/v1/events` intake
  (`event_emission: accepted|disabled|failed:<reason>`; ack never changed by emission
