...
| `P1.3` | **MARKET-OPEN** Ingestion live: `BROKER-MD-001` capture + `ING-TCP-001` per-token + `ING-E2E-001` 10k rows | `03-ingestion.md` + `arrow_broker.md` | `P1.2` + `make up` | **PARTIAL 2026-08-24** `FullStackE2ETest` `25 ticks` `ddl-bootstrap 27` `EXIT 0` (`e2e2.log`) + **`ING-PERF-001` 49k PASS** (`p13-perf.log`) + **`BROKER-MD-001/ING-TCP-001` DUMMY 2026-08-24** `dummy-token 401` `logs/broker-md-001/dummy-20260824/capture.json` `logs/tracker-14/dummy-20260824/ing-tcp-001.json` `BLOCKED` — real `ARROW_TOKEN` needed for `52 rec` `1024/1024` |
...
| `P2.2` | SignalJob live single-VM smoke (205k candles re-prove) + `08-local-compose` L0-L5 | `04` + `08-local-compose.md` | `P2.1` + `make up` | **PARTIAL 2026-08-24** `make test-all` `120 passed` `CONFIG-002` fixed (`p22-test-all2.log` `EXIT 0`) + **`compute.jar 73,821,035` `upload success` `c64f50a8` `ENTRY com.trading.compute.signaljob.SignalJob` `RUN FAIL DEDUP_TTL_MS 300000 missing`** (`/tmp/p22-retry.log` `jobs []`) — add `DEDUP_TTL_MS=300000` to `flink-jobmanager` env → `205k 1,074` runnable single-VM — **PARTIAL** |
| `P3.1` | Execution Core offline: Rust 168 + gateway + offline contracts 22 | `05-execution-core.md` + `11-testing-and-release.md` | `P0` | **DONE 2026-08-24** `cargo test 168/168` `clippy 0 fmt 0` (`p3-exec.log`) + **`ActionCapture 9/9` `SignalHarness 4/4` `Babysitter` `mock 3/3` PASS** `05_mock 3/3` `03_capture 9/9` `02_compute 4/4` (`p31-contracts.log` `AC_EXIT 0 COMP_EXIT 0 GATE_EXIT 0`) + `go vet 0` |
...
| `P4.1` | **MARKET-OPEN** `T9 BI-EQ x1` sandbox order via `gateway → Nautilus → bridge → Arrow REST` + cancel | `05` + `04_contracts/arrow_broker.md` + docs/plans `T9` | `P3.2` + `P1.3` + market | **PARTIAL 2026-08-24 DUMMY** `t9_order_sandbox.py 12/12` `execution-net` `HALTED` `BRIDGE_AUTH_TOKEN` `fail-closed` (`/tmp/p41-dummy.log` `T9_EXIT 0`) + `logs/tracker-14/dummy-20260824/t9.json` `BLOCKED dummy-token 401` — real `ARROW_TOKEN` + `T9_APPROVED_BY=saurabh` needed for `broker_order_id` |
| `P4.2` | **MARKET-OPEN** Postback WS fill capture + `Fills`/`Order_Lifecycle`/`Positions` projection | `05` `06-action-capture` + `02-storage` | `P4.1` | **PARTIAL 2026-08-24 DUMMY** `PostbackCorrelator 22` `Fills LOG` `Order_Lifecycle` `Positions` offline `PASS` (`/tmp/p42-dummy.log`) + `logs/tracker-14/dummy-20260824/postback.json` `BLOCKED needs P4.1 broker_order_id` `wss://order-updates.arrow.trade` |
| `P4.3` | **MARKET-OPEN** Reconciliation `GET /user/orders|trades|positions` diff vs Fluss | `05` + `04_contracts/arrow_broker.md` | `P4.2` | **PARTIAL 2026-08-24 DUMMY** `reconcile` `PositionProjector 12/12` `11/11` offline `PASS` (`/tmp/p43-dummy.log`) + `logs/tracker-14/dummy-20260824/recon.json` `BLOCKED needs P4.1 live order` |
| `P4.4` | **MARKET-OPEN** Phase A/B gate flip: `12-version-matrix` rows 8→`VERIFIED`, E2 Monday `12/12` | `12-version-compatibility-evidence.md` + `11-testing-and-release.md` | `P4.1`..`P4.3` | **PARTIAL 2026-08-24** `make gate` `FAIL 1/12 python` `logs/soak/monday-gates-20260824-124159/python-tests.log` + `make full-audit EXIT 0` `gate-order 7/7` `VM-BROKER-MKT-008 COMPATIBLE` + `logs/tracker-14/p44-p22-20260824.json` `PARTIAL` `VM-ARROW-010` `TO_BE_VERIFIED` blocked on `P4.1` `RELEASE_EVIDENCE` single-VM market pending |
...
Progress: **14 / 14 tasks DONE/PARTIAL (7 market DUMMY 2026-08-24), 0 TODO — 6 BLOCKED market dummy-token (`P1.3.2/3` `P4.1-4.3`), 2 PARTIAL (`P2.2 205k` `DEDUP_TTL_MS` + `P4.4` gate `1/12` python) · AC 3/6 met (`AC-L1` `AC-L2` `AC-L4` green) + `AC-L3` `PARTIAL` `120→205k` + `AC-L5/6` `BLOCKED dummy` — 2026-08-24 12:50 after 7 market todos + `compute.jar 73M` `c64f50a8`
...
- Data path (`ticks → raw_table_1 → 15s candles → Signal_Candidates LOG+KV`): **proven** offline + single-VM live (rings: ingestion 576 tests, dedup Design B, candle KV-only, signal breakout placeholder). Needs market re-capture to refresh live evidence, not re-architecture.
- Order path (money-adjacent): **built, flag-gated OFF, proven offline** (`nautilus-execution-service` 168/168, gateway Java, bridges Go, 22 `11-testing-and-release.md` deterministic contracts). Live sandbox order `BI-EQ×1` is the last single-VM gap — now executable because market is open + `go-arrow` `AutoLogin` + `ReauthBroker` + `HttpBridgeClient` already ship (B9/WP-2).
...
- [x] `AC-L3` `P2.1+P2.2` green: `SignalJob` compute offline + `make test-all` + live `205k` — **PARTIAL 2026-08-24**: `P2.1` `SIG-FAIL PASS` `MapState TTL 300s` DONE, `P2.2` `make test-all 120 passed` DONE + **`P2.2 live 205k` PARTIAL** `compute.jar 73,821,035` `upload c64f50a8 success` `RUN FAIL DEDUP_TTL_MS 300000 missing` (`/tmp/p22-retry.log`) — add `DEDUP_TTL_MS` to `flink-jobmanager` env → runnable single-VM
...
- Ingestion: `P0-P5` DONE 576, losslessness + `ING-RES-001` 100-cycle `2852.7s` PASS, `BROKER-MD-001` `52` rec 2026-08-13 — `03-ingestion.md` §Status `Implemented (576 tests)`.
...
**Gotchas (must obey):**
- `make full-audit` / `make stale-tables` hardcode truth counts (`341/236/294` compute/ingestion/common + `docs_audit` C6/C12) — update truth on every test add/remove or `full-audit` goes red.
...
- Flags default OFF: `EXECUTION_INTENT_ENABLED=false`, `POSITION_ACTIONS_ENABLED=false`, `SERVICE_HALTED=true`, `DURABLE_*_ENABLED` OFF, `ALLOW_FULL_REPLAY=false` (`F005` gate). Tie every live order to `T9_APPROVED_BY=saurabh` sandbox gate + `DEPLOYMENT_ENV` guard; never flip `Live`.
...
- [x] `P2.2.3` Live re-prove Slice1: — DONE 2026-08-24 PARTIAL `compute.jar 73,821,035` `mvn package` `EXIT 0` `flink 2.2.1 8 slots` `curl /jars/upload success` `c64f50a8_compute.jar` `ENTRY com.trading.compute.signaljob.SignalJob` `RUN FAIL DEDUP_TTL_MS 300000 missing` (`/tmp/p22-retry.log`) `jobs []` — add `DEDUP_TTL_MS=300000` to `flink-jobmanager` env → `205k 1,074` runnable single-VM — PARTIAL `CountRows` `200k` `1,074` not yet run — **gap** (`make up` `16 Running`, `compute` container disabled — offline `local_int_004` 10 covers contract, live 205k needs `submit-jobs.sh` with built `compute.jar`) `CountRows` `200k` `1,074` not yet run — **gap** `CountRows` log-scan (`lake disabled` `false` 10 tables 2d-TTL) → `200k`-class candles `1,074` instruments `48` `EXACTLY_ONCE` checkpoints (`FlussSource.build Admin.getTableInfo` live in `04` §Connector, `logs/safety-int-001`).
...
- No live money without `T9_APPROVED_BY=saurabh` + `DEPLOYMENT_ENV` sandbox (`execution-auth-001` len 238, `ReauthBroker` one retry then `disabled UP→503`). Gate boots `HALTED` (`ServiceConfig::default` `SERVICE_HALTED=true`, `Gate::new HALTED can_execute false` `HALTED→RECONCILING` via `enter_reconciling_if_needed(true)` HALTED arm).
