...
### Tests (current suite 184 green / 12 env-gated skips — see current-count note below; rows are historical)

> Rows below are the Slice-1-era record (2026-08-09). Current compute suite: **184 tests, 0 failures, 12 env-gated skips** (fresh full run 2026-08-13; the surefire headline includes the gated classes' skips); env-gated set (re-measured 2026-08-13, surefire): **12 skipped tests across 9 classes** — container battery (CandleGraphReplay 3, CandleFailureInjection 2, CandleRocksDbRestore 1, CandleTelemetryOutage 1, SignalJobOperatorUid 1, ComputeOtlpLiveDelivery 1 — the last skips without the collector reachable, SafetyHaltLiveIntegrationTest 1 — skips without `COMPUTE_INT_TEST_SAFETY`), plus `SignalJobObjectStoreCheckpointIntegrationTest` (1) and `SignalJobSavepointRestoreIntegrationTest` (1, host-runnable: embedded MiniCluster + `file://` savepoints, passes 2026-08-13). `SignalCurrentKvIdempotencyTest` is class-level env-gated (`COMPUTE_INT_TEST_SIGNAL_KV=true`) and reports **0 run / 0 skipped** without the env var — not part of the 12. Counts in rows that were superseded by the 2026-08-13 conversion are annotated below.
