...
| 3 | 30-min E2E soak (long-run acceptance) | `live-dev now` | **DONE as run, evidence-file GAP** — 19:43–20:14 IST `117 samples` `raw 35M→65M (+30M)` `feature 186k→310k (+123k)` `1024 inst` `179 EXACTLY_ONCE` `0 restarts/0 expired`, chk-1 2912 ms (stall fixed), last 775 MB 2478 ms; **files `e2e-30min-soak-20260817.md` + `o2-native-reporter-series-20260817.md` absent in `logs/tracker-14/` (8 files only, `16-…`/`p11-…` 68B placeholders) — run is evidenced via `/tmp/e2e-soak.log` + job `cfd15e7603c6d1eaac70abce8edf7665` but not force-added** | Gap: `logs/tracker-14/e2e-30min-soak-20260817.md` missing; `logs/tracker-14/o2-native-reporter-series-20260817.md` missing — to close, `git add -f` those two files |
...
- **3 (soak):** the 30-min soak is *run-DONE* (179 chks, 0 restarts) but *evidence-DONE is not* — the two `logs/tracker-14/` files the annotation cites are absent; the dossier is honest about the gap and points to `/tmp/e2e-soak.log` + the 30-min job id as interim evidence.
- **4 (telemetry):** sink-latency/restore/state-compat are *covered by native Flink metrics* (not by custom counters) — the `CLOSED-AS-COVERED` annotation *is* the implementation; the only missing artifact is the same O2 series file as Item 3.
