...
- `SIG-PERF-001` per-instrument workload envelope and p99 decision latency — **envelope half DONE 2026-08-17** (E2E measurement leg, see the Slice 1 evidence note); **p99 decision-latency half signal-side/pending** (out of scope — feature-path-only per user 2026-08-17).
...
**Remaining (tracker-14 P7-adjacent):** re-run the E2E on the Design-B topology —
...
(candle/signal/forming-bar Fluss ownership) is unchanged. **Open backlog:** the
P7.1/P7.2/P7.3 battery re-runs and the P10 rehearsal re-run execute on the current
(Design-B) topology; the open boxes remain the tracking surface.
...
re-run) now executes on the current Design-B topology.
