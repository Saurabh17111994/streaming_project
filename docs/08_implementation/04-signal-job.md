...
| `DEDUP_STATE_TABLE` (new, DEC-038) | Fluss KV state table name for the authoritative dedup set (`fingerprint_dedup`); validated at startup by the extended table preflight — DDL/manifest entry landed 2026-08-15 (24-table manifest) with the pin in §Design — `fingerprint_dedup` dedup state table; the live writer wiring remains evidence-gated |
