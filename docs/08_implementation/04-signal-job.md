...
| `S3_CHECKPOINT_URI_TO_BE_DEFINED` | Production encrypted checkpoint/savepoint storage — **DECIDED 2026-08-17: Cloudflare R2 (S3-compatible).** The concrete value is `CHECKPOINT_DIR=s3://tradingticks-aug-2026/flink-checkpoints` (same R2 bucket as the Iceberg lake, separate prefix), with `R2_ENDPOINT`/`AWS_ACCESS_KEY_ID`/`AWS_SECRET_ACCESS_KEY`/`AWS_REGION=auto` injected as secrets (tracker 14 P4.2; `SignalJobConfig` fails closed on an object-store URI without credentials; dev keeps `file://` overrides). Live proof: `SignalJobObjectStoreCheckpointIntegrationTest` (gate `COMPUTE_INT_TEST_P42=true`) — see the P4.2 evidence note |
...
- The tracker lives per-task in the job shell; ~~when the decision operators land it moves to broadcast state so `isTokenSuppressed` / `SuppressionGate` gate candidates/rankings/reservations/decisions~~ — **REMOVED 2026-08-15 (CHG-005 — decision operators out of scope)**. A tick alone never clears unsafe state; RECOVERED admits only post-recovery input. **Live-run finding 2026-08-18 (Item E):** because the tracker is per-task (not keyed/broadcast), a slot's UNSAFE and RECOVERED rows must be consumed by the SAME subtask — so the single-slot safety consumer runs at **parallelism 1**; at parallelism > 1 the pair can split across subtasks and the RECOVERED returns `IGNORED_NO_PRIOR_UNSAFE` (observed, correct, not a tracker bug). Evidence `logs/tracker-14/safety-live-job-run-20260818.md` §4.
- The job itself (live `FlussSource` consume path end-to-end) runs only after production approval; SAFETY-INT-001 already exercises the production bridge/parser/tracker against live Fluss. **DONE 2026-08-18 (Item E, explicit user approval):** the live job ran end-to-end against the dev Fluss — UNSAFE → `NEW_UNSAFE` (suppression) → RECOVERED → `RECOVERED` (admission) observed through the live consume path, `safety.transitions.applied` incremented, 0 malformed rows from the run, checkpoint retained on clean cancel (`RETAIN_ON_CANCELLATION`). The run also surfaced and fixed a latent submittability defect: `SlotAssignmentResolver` was not `Serializable` though it is carried as a Flink operator field, so the job had never been submittable — fixed (`SlotAssignment extends Serializable` + `SlotEntry` serializable) with a round-trip guard test; suites green (common 341/0/1, compute 292/0/17 at the 2026-08-18 Item E run — now 294/0/17 after the +2 forming-bar emission-profile tests, CHG-030 follow-up). Evidence `logs/tracker-14/safety-live-job-run-20260818.md`; change record CHG-026.
...
C/D: **(BOTH DONE 2026-08-17/18 — Item C: P7 battery re-run on Design-B, evidence
...
current (Design-B) topology per `18-signal-job-remaining-work-plan.md` Items C/D.
...
(`18-signal-job-remaining-work-plan.md` Items C/D; evidence in
`logs/tracker-14/p7-battery-design-b-20260817.md` +
`logs/tracker-14/p10-rehearsal-design-b-20260817.md`).
