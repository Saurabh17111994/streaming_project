...
| **SIGNAL-CHAIN-E2E-001 (full-chain live E2E — `SignalChainLiveE2ETest`, env-gated `SIGNAL_CHAIN_E2E=true`) — HARNESS LANDED 2026-08-17 (CHG-021); live run pending** — broker → `raw_table_1` → SignalJob → `feature_candles_15s` with the real `IngestionService` subprocess + real `buildTopology`; root cause fixed: local MiniCluster defaults `taskmanager.memory.managed.size` to 128 MB TOTAL → RocksDB block-cache starvation throttled the job to ≈ the feed rate, the backlog never drained, and the KV feature table stayed flat (every candle an upsert of a historical window). Ported from the FreeBuff design-B investigation (architecture reversal REJECTED 2026-08-17 — DEC-038 stays authoritative; preserved on `experiment/design-b-mapstate-dedup` for reference): `TASK_MANAGER_MEMORY_MANAGED_SIZE` / `TASK_MANAGER_NETWORK_MEMORY_MAX` passthrough in `applyRuntimeOptions` (unset → deployment authoritative), LOG `listOffsets` + KV max-of-3 count engine, daemon stream drains, runner env (real `INGESTION_CLASSPATH`, `STATE_BACKEND=rocksdb`, 2048m managed budget, 30 min default). Investigation: `logs/tracker-14/16-signal-chain-e2e-investigation-20260817.md`; runner: `code/01_platform/04_scripts/run-signal-chain-e2e.sh` (dev Fluss at localhost:9123) | Runner + dev Fluss cluster | 30-min live run: feature gains rows with `window_end` after run start, job stays RUNNING, ≥1 EXACTLY_ONCE checkpoint — **pending (next verification step, P7-adjacent)** |
