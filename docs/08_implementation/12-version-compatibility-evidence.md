...
| VM-IMAGES-012 Base images | COMPATIBLE_WITH_LIMITATION (2026-08-24 — digest+SBOM half; vuln-policy incomplete)  | `versions.pin` digests: `FLUSS_CLIENT 6921994a`, `FLUSS_FLINK 5dddeb`, `FLINK_STREAMING bb41cde`, `docker-compose.yml` `${FLUSS_IMAGE:?set ...}` digest-required, `pin-check.sh` no SNAPSHOT | 12/12 digests verified + SBOM attestations (4/5 images) + OS-layer trivy counts disclosed (`logs/version-matrix/images-012-20260824T171559Z.md`); library-layer scan-unavailable, SEC-IMAGE-001 threshold TO_BE_VERIFIED |
...
| VM-IMAGES-012 | `UNKNOWN` (partial evidence) | 2026-08-24: 12 image RepoDigests verified matching versions.pin/.env; buildx SBOM attestations for fluss/flink/zookeeper/golang (openobserve absent); trivy OS-layer HIGH/CRITICAL per image (python 50/3, golang 17/2, zookeeper 13/0, fluss 2/0, flink 0/0); library-layer scan-unavailable (Java DB network block) + SEC-IMAGE-001 threshold TO_BE_VERIFIED (`logs/version-matrix/images-012-20260824T171559Z.md`, `PARTIAL_EVIDENCE_DIGEST_SBOM_OS_ONLY`) |
