...
2. **Compute module is NOT in the root Maven reactor (R-272).** Always run its tests module-locally:
   ...
   ```
   The suite must be green after every code change: **292 run / 0 failures** (15 skips without gates + 2 class-gated classes that report 0/0 = the documented "17 env-gated skips").
