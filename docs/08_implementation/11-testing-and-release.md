...
| `InMemoryAttemptStoreTest` | 8 | SCH-15 guard first consumer / EXE-UNIT-006 core — `InMemoryAttemptStore.prepare()` runs the attempt-store matrix guard (`checkWrite`): duplicate (instruction_id, request_hash) returns the existing PREPARED attempt untouched, a modified decision under an existing instruction_id is a contract violation + halt with no mutation, identity and broker-adapter-group columns are rejected, and every mint self-asserts its mutable group |
