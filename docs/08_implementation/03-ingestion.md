...
- Mode-coverage: byte-for-byte golden fixtures exist for FULL and LTP frames only; LTPC and QUOTE frames decode via `parseLTPC`/`parseQuote` (vendored SDK `streams.go`). Decode + emission coverage added 2026-08-13: `third_party/go-arrow/arrow/streams_test.go` pins all four `ParseMarketTick` payload layouts (13/17/93/241 B, incl. the R-203 NetChange-vs-LTQ rule) and `go-bridge/main_test.go` pins `fromStandardTick` emission (feed/mode passthrough, TS LTT*1000 with Time fallback, OI zero-guard, depth truncated to 5). Standard-stream wire fixtures remain pending live sandbox frames (BROKER-MD-001).
