...
| Field | Value |
| --- | --- |
| Status | Implementation active; Phases 2a-2g complete; 300 tests (188 ingestion + 112 common), 0 failures, 7 env-gated skips. Open items: ING-PERF-001 50k gate certified 2026-08-13 at the synthetic hot-path envelope (socket 49,242 tps / 0 wire loss; append hot path 49,578 tps / 0 failures / p99 &lt; 5 ms — floors 47.5k/48k PASS); the 30-min 3,000-instrument production-envelope run is REMOVED from acceptance (DEC-037, 2026-08-13 — user decision: not to be tested); ING-RES-001 100-cycle real-backoff soak PASS 2026-08-13 (100/100 cycles, 2852.7 s, goroutines 3 &lt; baseline 4, fds 7 = baseline — no leak; evidence logs/res001-real-backoff/res001-real-backoff-20260813.out); BROKER-MD-001 live evidence captured 2026-08-13 (real wire on both feeds: standard 13/17/93/249 B big-endian + HFT 40/196 B zstd; AutoLogin/TOTP verified programmatically, no device token; full-mode 249 B confirmed — 241 B legacy retired; VM-BROKER-MKT-008 COMPATIBLE; evidence logs/broker-md-001/); residual: CAS trailer not yet observed live (unit-tested only); 90k peak campaign retired (DEC-036); standard-feed E2E fix landed 2026-08-13 (commit `syq`): the standard path now emits the full v2 NDJSON contract via `EmitTick` (raw_payload/payload_hash present) — pre-fix every standard tick was quarantined `HASH_MISMATCH`; post-fix the payload hash validates and the freshness gate is reached (post-close ticks quarantine as `STALE_BROKER_TIMESTAMP`; live `raw_table_1` writes verified pending market hours 2026-08-14 09:15 IST); ING-TCP-001 count-based losslessness: `ARROW_TICK_COUNTS` per-token emitted-tick counters (env-gated, stderr report, chunked) + `TokenCountReconcile` probe + `reconcile-compare.py` — validation PASS 2026-08-13 (HFT-1024 × 3 epochs: 3,072 sink rows = 3 × 1,024 bridge ticks; 0 lost / 0 extra / 0 vanished; evidence `logs/tracker-14/losslessness-validation-20260813.md`); market-hours 30–60 min proof pending (next window Aug 14); standard mode at the full 1,024-token manifest hits broker close 1006 (breakpoint between 3 and 1,024 tokens unknown) — per-connection cap / multi-connection fan-out needed before full-manifest standard mode is viable; HFT mode handles 1,024 tokens |
...
- `ING-TCP-001` count-based broker TCP losslessness: 30–60 min market-hours run on the single production connection; per-token bridge emitted-tick counts (`ARROW_TICK_COUNTS`, stderr reports incl. final at shutdown) reconciled against per-token Fluss rows (`TokenCountReconcile` probe: raw_table_1 LogScanner + ingestion_quarantine BatchScanner; `reconcile-compare.py --exact`) — exact equality per token proves no loss. Instrumentation validated live 2026-08-13 post-close (3 HFT-1024 epochs: 3,072 emitted = 3,072 stored; evidence `logs/tracker-14/losslessness-validation-20260813.md`).
...
- Mode-coverage: byte-for-byte golden fixtures exist for FULL and LTP frames only; LTPC and QUOTE frames decode via `parseLTPC`/`parseQuote` (vendored SDK `streams.go`). Decode + emission coverage added 2026-08-13: `third_party/go-arrow/arrow/streams_test.go` pins all `ParseMarketTick` payload layouts (13/17/93/249 B, 241 B legacy, 29/33/109/265 B CAS trailer, incl. the R-203 NetChange-vs-LTQ rule) and `go-bridge/main_test.go` pins `fromStandardTick` emission (feed/mode passthrough, TS LTT*1000 with Time fallback, OI zero-guard, depth truncated to 5). Standard-stream wire fixtures captured live 2026-08-13 (BROKER-MD-001): real ds.arrow.trade frames recorded raw (13/17/93/249 B + 1 B heartbeats) in `logs/broker-md-001/` and embedded as non-circular fixtures in the SDK tests; full-mode 249 B (docs) confirmed live — the legacy 241 B layout is retired; the vendored SDK also landed the AutoLogin auth fix (validate-2fa host + appID field) and the 249/CAS parse fix (production `runStandard` died on every 249 B tick before it).
