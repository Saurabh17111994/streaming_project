# Action Capture and Position Projection Implementation Dossier
...
| Requirements | `REQ-AC-001`–`REQ-AC-010` |
| Contract | `docs/04_contracts/06-action-capture.md` |
| Writes | `Fills_table`, `Order_Lifecycle`, `Positions`, `Postback_Quarantine` |
...
└─ correlated
   → immutable Fills_table append
   → durable projection work record
...
- `MAX_PENDING_PROJECTION_RECORDS`
