# Local Compose Implementation Dossier
...
| OpenAlgo | Sandbox API responds | Contract probe passes; no live order |
...
Bind operator interfaces to localhost by default. Do not expose Fluss internal RPC, Flink administrative APIs, or OpenAlgo order APIs beyond the local test boundary.
