...
| SCH-15 | Partial-update merge semantics: column ownership, merge engine behaviour | [ ] | **Partial (2026-08-15)** — the storage-layer merge behaviour is now observed (COMPAT-FLUSS-003 changelog FULL-image test: a `partial_update` merges at the storage layer and the changelog record carries the merged FULL image; untouched columns preserved). The column-ownership writer half remains open — no production `partial_update` writer exists (depends on Executor-era writers; maps COMPAT-FLUSS-004). |
