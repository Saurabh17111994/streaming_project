...
| G5 | Process-level Go bridge tests are minimal | Medium | **PARTIALLY RESOLVED in M2 (2026-08-15)** — ING-UNIT-018 added 13 exec-self subprocess FATAL cases to `hft_policy_test.go` (wrong pins incl. `ARROW_HFT_CONNECTIONS=2`, non-integers, out-of-range tunables → exit 2), so pin enforcement and FATAL-exit codes ARE process-tested. Remainder (M5, ING-UNIT-020): the missing-`ARROW_APP_ID` startup FATAL and asserting the documented FATAL message text — the exec-self cases assert exit code 2 only. |
...
| `ING-FAIL-006` | P3 | 30 s warning-window throttle unpinned | Re-trigger the ≥80% warning within 30 s (records then bytes) | The warning listener fires at most once per 30 s window — the ≥80% readiness-false half is already covered by `warningAt80PercentRecords`/`warningAt80PercentBytes` | extend `AppendTrackerTest` — **M5 backlog (folded in at the 2026-08-15 audit)** |
...
| `ING-UNIT-020` | P3 | G5 — process-level FATAL exits (remainder) | Run the real binary with missing `ARROW_APP_ID`; assert the documented FATAL message text on the exec-self cases | Exit code 2 + the documented FATAL message for each case (wrong pins + `ARROW_HFT_CONNECTIONS=2` are already covered by ING-UNIT-018's exec-self cases — the open slice is `ARROW_APP_ID` + message assertions) | extend `hft_policy_test.go` — **M5 backlog (folded in at the 2026-08-15 audit)** |
...
| **M5 — Telemetry + ops + remaining polish** | `ING-UNIT-021`, `ING-UNIT-022`, `ING-INT-006`, `ING-FAIL-006`, `ING-UNIT-020` | Metrics scrubbing + cardinality (ING-UNIT-021/022), docker-entrypoint exit codes (ING-INT-006), the 30 s warning-window throttle (ING-FAIL-006), and the G5 remainder — missing-`ARROW_APP_ID` + FATAL-message assertions (ING-UNIT-020). The last two were folded into M5 at the 2026-08-15 audit (previously in the inventory but assigned to no milestone). |
